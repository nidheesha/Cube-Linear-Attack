...
class BlackBoxPoly:
    def __init__(self,equation, degree=3 ):


        args = Parser().args
        #terms = args.equation.split(" ");
        terms = equation.split(" ");
        self.degree = degree

        self.publicvariables = []
        self.secretvariables = []
#public variables and private varialbles
...
#generating max terms
        ...
        self.coefficients = {}
####changed        
#giving random coefficents to max terms
        for maxterm in self.maxterms:
            self.coefficients[maxterm] = 0

        for term in terms:
            self.coefficients[term] = 1; 
            
        #    k += 1
#taking random private key
        self.private_key = np.random.randint(0, 2, degree)

        debug_string = ""

        for key_max in self.coefficients.keys():
#if coefficients of a max term is one then add it to the debug string which is the complete string 
    ...
    def evalonline(self, assignment_dict):
        public_assignment = {}
        private_assignment = {}
#setting all the public variables to zero as initialized value and private variables to chosen private key
        for i in range(self.degree):
            public_assignment[self.publicvariables[i]] = 0
            private_assignment[self.secretvariables[i]] = self.private_key[i]
#set the public variables and private variables to the assigned values
